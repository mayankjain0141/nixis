...
func TestCEL_Evaluate_AllowRule(t *testing.T) {
	...
	val, err := builder.Evaluate(context.Background(), prog, req, verdict, decodeArgs(t, req.Args))
	if err != nil {
...
func TestCEL_Evaluate_AllowRule_FalseForOtherTool(t *testing.T) {
	...
	val, err := builder.Evaluate(context.Background(), prog, req, classify.VerdictEntry{RiskLevel: classify.RiskLow}, decodeArgs(t, req.Args))
	if err != nil {
		t.Fatalf("Evaluate: %v", err)
	}
	if val != types.False {
...
func TestCEL_Evaluate_CustomFunctions(t *testing.T) {
	...
	t.Run("label.dominates true", func(t *testing.T) {
		...
		val, err := builder.Evaluate(context.Background(), prog, req, classify.VerdictEntry{}, decodeArgs(t, req.Args))
		if err != nil {
			t.Fatalf("Evaluate: %v", err)
		}
		if val != types.True {
...
func TestCEL_FindSearchRoot_AbsolutePath(t *testing.T) {
	...
	val, evalErr := builder.Evaluate(context.Background(), prog, req, classify.VerdictEntry{}, decodeArgs(t, req.Args))
	if evalErr != nil {
...
func TestCEL_EvalDeterministic(t *testing.T) {
	...
	var prev string
	for i := 0; i < 1000; i++ {
		val, err := builder.Evaluate(context.Background(), prog, req, verdict, decodedArgs)
		if err != nil {
...
func TestCEL_SetSessionLabels_ConcurrentSafe(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func() {
			for j := 0; j < 20; j++ {
				_, _ = builder.Evaluate(context.Background(), prog, req, classify.VerdictEntry{}, decodeArgs(t, req.Args))
			}
		}()
	}
